            ...
            Section(stringResource(R.string.filter_section_amount)) {
                Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    LaunchedEffect(filter.amountMin) {
                        if (filter.amountMin != minText.toDoubleOrNull()) minText = filter.amountMin?.let { formatAmount(it) } ?: ""
                    }
                    LaunchedEffect(filter.amountMax) {
                        if (filter.amountMax != maxText.toDoubleOrNull()) maxText = filter.amountMax?.let { formatAmount(it) } ?: ""
                    }
                    OutlinedTextField(
                        value = minText,
                        onValueChange = { raw ->
