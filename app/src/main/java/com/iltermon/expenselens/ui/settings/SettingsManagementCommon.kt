...
internal fun LimitSubtitle(limitMonthly: Double?, limitYearly: Double?) {
    ...
    if (limitMonthly != null) parts.add(stringResource(R.string.limit_per_month,
        money(limitMonthly, 0)
    ))
    if (limitYearly != null) parts.add(stringResource(R.string.limit_per_year,
        money(limitYearly, 0)
    ))
    Text(
